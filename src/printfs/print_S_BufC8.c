...
/* ------------------------------- CpyTail_BufC8 ------------------------------------------
   ...
   Return 'dest->cnt' the number of chars copied' into dest; '\0' if 'key' not found.
*/
PUBLIC S_BufC8 const * CpyTail_BufC8(S_BufC8 *dest, S_BufC8_ro const *src, C8 const *key) {
   /* If both 'src' and 'key are non-NULL AND non-empty the proceed with the copy.
      Otherwise return 'dest' unchanged.
   */
   if(src != NULL && src->cs != NULL && src->cs[0] != '\0' && src->cnt > 0 &&
      key != NULL && key[0] != '\0') {
      C8 const *keyAt = strnstr(src->cs, key, src->cnt);    // Look for 'key' in 'src'.

      if(keyAt != NULL) {                                   // Found 'key'?
         U16 tailChars = src->cnt - (keyAt - src->cs);      // Tail, including 'key' is these many.
...
/* ------------------------------------- Strlen_BufC8 ------------------------------------------
   ...
   If 'src' is NULL of empty ("") then return 0.
...
/* ------------------------------------ Strcpy_BufC8 --------------------------------------------
   ...
   If 'src' is NULL or empty ("") return 'dest' unchanged.
*/
PUBLIC S_BufC8 const * Strcpy_BufC8(S_BufC8 const *dest, C8 const *src) {
   if(src != NULL && src[0] != '\0') {                // 'src' is not NULL? AND 'src' is not empty?
...
/* ----------------------------------- Strcat_BufC8 --------------------------------------------
   ...
   Return 'dest'; if 'src' is NULL or empty ("") then return 'dest' unchanged.
*/
...
/* ------------------------------------- StrChainCpy_BufC8 --------------------------------------------
   ...
   If 'src' is NULL or empty ("") then return a copy of 'dest' as-is.
*/
PUBLIC S_BufC8 Strcpy_ChainBufC8(S_BufC8 const *dest, C8 const *src) {
   if(src == NULL || src[0] == '\0') {             // 'src' is NULL? OR 'src' is empty?
      return (S_BufC8){.cs = dest->cs, .cnt = dest->cnt}; }    // then return a copy of 'dest' as-is.
   else {
      U16 nChars = MinU16(dest->cnt, strlen(src));    // Will copy up what will fit in 'dest'.
