...
PUBLIC bool heapStr_Write(S_HeapStr *s, C8 const *src) {
   ...
   if(len <= maxStr) {                                   // 'src' fits in 's'
      return safeRef_Write(s, (U8 const*)src, len+1); }  // then write all of it.
      ...
      bool rtn = safeRef_Write(s, (U8 const*)p, maxStr+1);
      free(p);
      return rtn; }}

// ----------------------------------------------------------------------------------------
PUBLIC C8 const * heapStr_Read( S_HeapStr *s, C8 *out, U16 charsReq) {
   if(true == safeRef_Read(s, (U8*)out, MinU16(safeRef_Used(s), charsReq))) {
      out[charsReq] = '\0'; }
   else {
