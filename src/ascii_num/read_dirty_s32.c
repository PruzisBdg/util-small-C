...
|  ReadAsciiS32()
|
|  The same as ReadDirtyASCII_S32() above, but must start on a +-digit; does not eat leading
|  chars.
|
------------------------------------------------------------------------------------------*/
      ...
      else                             // otherwise keep going
      {
         if( !isdigit(ch) )            // This char isn't a digit?
         {
            ...
            else                       // else we don't have number yet. Keep going
            {
               if(ch == '-')           // Negative (perhaps)?
               {
                  if(isNeg == 1 || isPos == 1)       // But already got '-' or '+'
                  {
                     return NULL;      // So fail, allow just one '-' directly in front of 1st digit.
                  }
                  else
                  {
                     isNeg = 1;        // then mark that, it may in front of the 1st digit of new number
               ...
               else if(ch == '+')      // else positive
               {
                  ...
                  {
                     isPos = 1;           // so cancel 'neg' (if it was set)
                  }
               }
               else
               {
                  return NULL;         // else not leading '+' or '-'
               }
            }
         }
         else                          // else this char is a digit
